...
-- Added E_INSERT for SE compatibility
%token <ET_KEYWORD> E_IF E_INDEXING E_INFIX E_INHERIT E_INSPECT E_INSERT
...
%token <ET_SYMBOL> E_ASSIGN_CHECK E_ASSIGN_FORCE
...
%token <ET_SYMBOL> '{' '}'
%token <ET_SYMBOL> '(' ')' ':' ',' ']' '$' '.' '!'	-- '~' removed
%token <ET_SYMBOL_OPERATOR> '-'
%token <ET_SYMBOL_OPERATOR> '+'
%token <ET_SYMBOL_OPERATOR> '~'		-- SE Compatibility
%token <ET_SYMBOL> '='
...
%type <ET_CHOICE_CONSTANT> Choice_constant Choice_string
...
--## Added Inherit_non_conforming to next line
--%type <ET_KEYWORD> Inherit_non_conforming	--???

--####
%type <ET_PARENT_LIST> Inherit_continue Inherit_continue_end
%type <ET_KEYWORD> Inherit
%type <ET_MANIFEST_ARRAY> Typed_manifest_array
--####
...
Obsolete_opt: -- Empty
	...
	| E_OBSOLETE E_ONCE_STRING Manifest_string				-- For SE compatibility
		{ $$ := ast_factory.new_obsolete_message ($1, $3) }
	;
...
-- Optional inheritance section, #not# ending in 'end'
...
Inheritance_opt: -- Empty
	...
	| Inherit_first Parent_list
		{	-- OK
			$$ := $2
			remove_counter
		}
	| Inherit_first Inherit_continue
		{
			$$ := $2
			remove_counter
		}
...
Inheritance_end: Inherit_first Parent_list_end
		{
			$$ := $2
			remove_keyword
			remove_counter
		}
	| Inherit_first Inherit_continue_end
		{
			$$ := $2
			remove_keyword
			remove_counter
...
Inherit: E_INHERIT
			{ $$ := $1; set_inherit_non_conforming(False) }
	| E_INHERIT '{' '}'
			{ $$ := $1; set_inherit_non_conforming(True) }
	| E_INHERIT '{' E_IDENTIFIER '}'
			{ $$ := $1; set_inherit_non_conforming(True) }
	| E_INSERT
			{ $$ := $1; set_inherit_non_conforming(True) }
...
Inherit_continue: Inherit Parent_list
		{ $$ := $2 }
	| Parent Inherit_continue
		{
			$$ := $2
			if $$ /= Void and $1 /= Void then
				$$.put_first ($1)
	...
	| Parent_semicolon Inherit_continue
		{
			$$ := $2
			if $$ /= Void and $1 /= Void then
				$$.put_first ($1)
...
Inherit_continue_end: Inherit Parent_list_end
		{ $$ := $2 }
	| Parent Inherit_continue_end
		{
			$$ := $2
			if $$ /= Void and $1 /= Void then
				$$.put_first ($1)
	...
	| Parent_end Inherit_continue_end
		{
			$$ := $2
			if $$ /= Void and $1 /= Void then
				$$.put_first ($1)
	...
	| Parent_semicolon Inherit_continue_end
		{
			$$ := $2
			if $$ /= Void and $1 /= Void then
				$$.put_first ($1)
...
--####
	| Writable E_ASSIGN_FORCE Expression
		{ $$ := ast_factory.new_assignment ($1, $2, $3) }	-- new_assign_force
--####
...
Choice_string: Manifest_string
--		{ $$ := ast_factory.new_choice_string ($1) }
	;
------------------------------------------------------------------------------------
...
Binary_expression: Expression E_FREEOP Expression
	...
	| Expression E_IMPLIES Expression
		{ $$ := ast_factory.new_infix_expression ($1, $2, $3) }
	| Writable E_ASSIGN_CHECK Expression
		{ $$ := edp_ast_factory.new_assign_check ($1, $2, $3) }	-- new_assign_check
	| Type E_ASSIGN_CHECK Expression
		{ $$ := edp_ast_factory.new_assign_type_check ($1, $2, $3) }	
	;
...
--##### Added '~' SE compatibility complement operator
	| '~' Non_binary_expression %prec E_NOT
...
Manifest_type: '{' Type '}'
	...
	;
--################################################################################################################
Typed_manifest_array: '{' Type E_INTEGER ',' Manifest_array '}'
		{
			$$ := $5
			-- TODO
			-- Set type of array
...
Typed_integer_constant: '{' Type '}' Untyped_integer_constant
	...
	|	'{' Type Untyped_integer_constant '}'
		-- SE Version
		{	$$ := $3
			$$.set_cast_type (ast_factory.new_target_type ($1, $2, $4))
