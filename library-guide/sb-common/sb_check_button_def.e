...
feature -- Queries
   ...
   get_check_button_style: INTEGER
...
feature -- Actions
   ...
   set_check_button_style (style: INTEGER)
...
feature -- Message processing
   ...
   on_update (sender: SB_MESSAGE_HANDLER; selector: INTEGER; data: ANY): BOOLEAN is
      do
         if not Precursor(sender,selector,data) then
            ...
            if (options & CHECKBUTTON_AUTOGRAY) /= 0 then
               disable
            end
         end
         Result := True
   ...
   on_unknown (sender: SB_MESSAGE_HANDLER; selector: INTEGER; data: ANY): BOOLEAN
      do
         set_state (SB_MAYBE)
         Result := True
      end
