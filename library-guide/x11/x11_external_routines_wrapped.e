...
feature -- X11 Routines and Functions
	...
	XWhitePixel, x_white_pixel(display: POINTER; screen: INTEGER): INTEGER is
		do
	...
	XConnectionNumber, x_connection_number(display: POINTER): INTEGER is
		-- The file descriptor by which the X display is accessed
		do
			Result := c_XConnectionNumber(display)
		end

	XDefaultColormap, x_default_colormap(display: POINTER; screen: INTEGER): INTEGER is
		do
	...
	XDefaultDepth, x_default_depth(display: POINTER; screen: INTEGER): INTEGER is
		do
	...
	XListDepths, x_list_depths(display: POINTER; screen: INTEGER; count_return: POINTER): POINTER is
			-- 
		do
			Result := c_XListDepths(display, screen, count_return)
		end

	XDefaultGC, x_default_gc(display: POINTER; screen: INTEGER): POINTER is
	...
	XDefaultRootWindow, x_default_root_window(display: POINTER): INTEGER is
		do
	...
	XDefaultScreenOfDisplay, x_default_screen_of_display(display: POINTER): POINTER is
		do
	...
	XInternAtom, x_intern_atom (d, s: POINTER; b: BOOLEAN): INTEGER is
		do
			Result := c_XInternAtom(d, s, b)
    	end

	XGetAtomName, x_get_atom_name (d: POINTER; i: INTEGER): POINTER is
		do
			Result := c_XGetAtomName(d, i)
    	end

  	XCloseDisplay, x_close_display (ptr: POINTER) is
		do
			c_XCloseDisplay(ptr)
    	end
  
	XDisplayString, x_display_string (ptr: POINTER): POINTER is
		do
			Result := c_XDisplayString(ptr)
    	end

	XScreenCount, x_screen_count (disp: POINTER): INTEGER is
		do
			Result := c_XScreenCount(disp)
		end

	XDefaultScreen, x_default_screen (disp: POINTER): INTEGER is
		do
	...
	XDisplayWidth, x_display_width (disp: POINTER; scr: INTEGER): INTEGER is
		do
  	...
  	XDisplayHeight, x_display_height (disp: POINTER; scr: INTEGER): INTEGER is
		do
  	...
  	XDisplayWidthMM, x_display_width_mm (disp: POINTER; scr: INTEGER): INTEGER is
		do
	...
	XDisplayHeightMM, x_display_height_mm (disp: POINTER; scr: INTEGER): INTEGER is
		do
  	...
  	XDisplayCells, x_display_cells (d: POINTER; scr: INTEGER): INTEGER is
		do
  	...
  	XDefaultVisual, x_default_visual (disp: POINTER; scr: INTEGER): POINTER is
		do
...
--    	end
  	...
  	XRootWindow, x_root_window (disp: POINTER; scr: INTEGER): INTEGER is
		do
...
--    	end
  	...
  	XFlush, x_flush (disp: POINTER) is
		do
			c_XFlush(disp)
    	end

  	XSync, x_sync (disp: POINTER; b: BOOLEAN) is
		do
			c_XSync(disp, b)
    	end

  	XProtocolVersion, x_protocol_version (disp: POINTER): INTEGER is
		do
			Result := c_XProtocolVersion(disp)
    	end

  	XProtocolRevision, x_protocol_revision (disp: POINTER): INTEGER is
		do
			Result := c_XProtocolRevision(disp)
    	end

  	XVendorRelease, x_vendor_release (disp: POINTER): INTEGER is
		do
			Result := c_XVendorRelease(disp)
    	end

  	XServerVendor, x_server_vendor (disp: POINTER): POINTER is
		do
			Result := c_XServerVendor(disp)
    	end

  	XSetCloseDownMode, x_set_close_down_mode (disp: POINTER; i: INTEGER) is
		do
  	...
  	XKillClient, x_kill_client (disp: POINTER; i: INTEGER) is
		do
			c_XKillClient(disp, i)
    	end

  	XListFonts, x_list_fonts (disp, str: POINTER; i: INTEGER; p: POINTER): POINTER is
		do
		--	fx_trace(0, <<"XListFonts: Serial: ", c_XNextRequest(disp).out>>)
			Result := c_XListFonts(disp, str, i, p)
    	end

  	XGetFontPath, x_get_font_path (disp: POINTER; p: POINTER): POINTER is
		do
			Result := c_XGetFontPath(disp, p)
    	end

  	XSetFontPath, x_set_font_path (disp: POINTER; p: POINTER; i: INTEGER) is
		do
			c_XSetFontPath(disp, p, i)
    	end

  	XBell, x_bell (disp: POINTER; i: INTEGER) is
		do
			c_XBell(disp, i)
    	end

  	XAutoRepeatOn, x_auto_repeat_on (disp: POINTER) is
		do
			c_XAutoRepeatOn(disp)
    	end

  	cc_XAutoRepeatOff, x_auto_repeat_off (disp: POINTER) is
    	external "C use <X11/Xlib.h>"
    	alias "XAutoRepeatOff"
    	end

  	cc_XScreenOfDisplay, x_screen_of_display (disp: POINTER; i: INTEGER): POINTER is
    	external "C use <X11/Xlib.h>"
...
--    	end
  	...
  	cc_XMaxRequestSize, x_max_request_size (disp: POINTER): INTEGER is
    	external "C use <X11/Xlib.h>"
    	alias "XMaxRequestSize"
    	end

  	cc_XExtendedMaxRequestSize, x_extended_max_request_size (disp: POINTER): INTEGER is
    	external "C use <X11/Xlib.h>"
    	alias "XExtendedMaxRequestSize"
    	end

  	cc_XLastKnownRequestProcessed, x_last_known_request_processed (disp: POINTER): INTEGER is
    	external "C use <X11/Xlib.h>"
    	alias "XLastKnownRequestProcessed"
    	end

  	cc_XNextRequest, x_next_request (disp: POINTER): INTEGER is
    	external "C use <X11/Xlib.h>"
    	alias "XNextRequest"
    	end

	cc_XQLength, x_q_length (disp: POINTER): INTEGER is
    	external "C use <X11/Xlib.h>"
    	alias "XQLength"
    	end

  	cc_XStoreBytes, x_store_bytes (disp, buf: POINTER; n: INTEGER) is
    	external "C use <X11/Xlib.h>"
    	alias "XStoreBytes"
    	end

  	cc_XFetchBytes, x_fetch_bytes (disp, n: POINTER): POINTER is
    	external "C use <X11/Xlib.h>"
    	alias "XFetchBytes"
    	end

  	cc_XStoreBuffer, x_store_buffer (disp, buf: POINTER; n, nb: INTEGER) is
    	external "C use <X11/Xlib.h>"
    	alias "XStoreBuffer"
    	end

  	cc_XFetchBuffer, x_fetch_buffer (disp, n: POINTER; nb: INTEGER): POINTER is
    	external "C use <X11/Xlib.h>"
    	alias "XFetchBuffer"
    	end

  	cc_XRotateBuffers, x_rotate_buffers (disp: POINTER; nb: INTEGER) is
    	external "C use <X11/Xlib.h>"
...
feature -- Event Handling
	...
	cc_XNextEvent, x_next_event (d, buf: POINTER) is
		external "C use <X11/Xlib.h>"
    	alias "XNextEvent"
    	end

	cc_XPeekEvent, x_peek_event (d, buf: POINTER) is
    	external "C use <X11/Xlib.h>"
    	alias "XPeekEvent"
    	end
  	...
  	cc_XCheckMaskEvent, x_check_mask_event (d: POINTER; msk: INTEGER; buf: POINTER): INTEGER is
    	external "C use <X11/Xlib.h>"
    	alias "XCheckMaskEvent"
    	end

  	cc_XCheckTypedEvent, x_check_typed_event (d: POINTER; typ: INTEGER; buf: POINTER): INTEGER is
    	external "C use <X11/Xlib.h>"
    	alias "XCheckTypedEvent"
    	end

  	cc_XWindowEvent, x_window_event (d: POINTER; wid, msk: INTEGER; buf: POINTER) is
    	external "C use <X11/Xlib.h>"
    	alias "XWindowEvent"
    	end

  	cc_XMaskEvent, x_mask_event (d: POINTER; msk: INTEGER; buf: POINTER) is
    	external "C use <X11/Xlib.h>"
    	alias "XMaskEvent"
    	end

  	cc_XPutBackEvent, x_put_back_event (d: POINTER; buf: POINTER) is
    	external "C use <X11/Xlib.h>"
    	alias "XPutBackEvent"
    	end
  	...
  	cc_XAllowEvents, x_allow_events (d: POINTER; type, time: INTEGER) is
  		external "C use <X11/Xlib.h>"
    	alias "XAllowEvents"
    	end

  	cc_XEventsQueued, x_events_queued (d: POINTER; m: INTEGER): INTEGER is
    	external "C use <X11/Xlib.h>"
    	alias "XEventsQueued"
    	end
...
feature -- XContext routines
	...
	cc_XrmUniqueQuark, x_create_context: INTEGER is
		external
			"C use <X11/Xlib.h>"
			alias "XrmUniqueQuark"
		end

	cc_XSaveContext, x_save_context(disp: POINTER; w, context: INTEGER; pw: POINTER): INTEGER is
		external
			"C use <X11/Xlib.h>"
			alias "XSaveContext"
		end

	cc_XFindContext, x_find_context(disp: POINTER; w, context: INTEGER; pres: POINTER): INTEGER is
		external
			"C use <X11/Xlib.h>"
			alias "XFindContext"
		end

	cc_XDeleteContext, x_delete_context(disp: POINTER; w: INTEGER; context: INTEGER): INTEGER is
		external
			"C use <X11/Xlib.h>"
...
feature -- GC routines
  	...
  	cc_XCreateGC, x_create_gc (d: POINTER; wid, mask: INTEGER; p: POINTER): POINTER is
    	external "C use <X11/Xlib.h>"
    	alias "XCreateGC"
    	end

  	cc_XFreeGC, x_free_gc (d, ctxt: POINTER) is
    	external "C use <X11/Xlib.h>"
    	alias "XFreeGC"
    	end

  	cc_XGetGCValues, x_get_gc_values (d, ctxt: POINTER; m: INTEGER; v: POINTER) is
    	external "C use <X11/Xlib.h>"
    	alias "XGetGCValues"
    	end

  	XChangeGC, x_change_gc (d, ctxt: POINTER; m: like long_anchor; v: POINTER) is
		do
		--	fx_trace(0, <<"XChangeGC: Serial: ", c_XNextRequest(d).out>>)
			c_XChangeGC(d, ctxt, m, v)
    	end

  	cc_XSetFont, x_set_font (d, ctxt: POINTER; fid: INTEGER) is
   	 	external "C use <X11/Xlib.h>"
    	alias "XSetFont"
    	end

  	XSetBackground, x_set_background (d, ctxt: POINTER; pix: INTEGER) is
		do
		--	fx_trace(0, <<"XSetBackground: Serial: ", c_XNextRequest(d).out>>)
			c_XSetBackground(d, ctxt, pix)
    	end

  	cc_XSetForeground, x_set_foreground (d, ctxt: POINTER; pix: INTEGER) is
    	external "C use <X11/Xlib.h>"
    	alias "XSetForeground"
    	end

  	XSetFunction, x_set_function (d, ctxt: POINTER; f: INTEGER) is
		do
			c_XSetFunction(d, ctxt, f)
    	end

  	cc_XSetLineAttributes, x_set_line_attributes (d, ctxt: POINTER; w, l, b, j: INTEGER) is
    	external "C use <X11/Xlib.h>"
    	alias "XSetLineAttributes"
    	end

  	cc_XSetArcMode, x_set_arc_mode (d, ctxt: POINTER; f: INTEGER) is
    	external "C use <X11/Xlib.h>"
    	alias "XSetArcMode"
    	end

  	cc_XSetFillRule, x_set_fill_rule (d, ctxt: POINTER; r: INTEGER) is
    	external "C use <X11/Xlib.h>"
    	alias "XSetFillRule"
    	end

  	XSetFillStyle, x_set_fill_style (d, ctxt: POINTER; s: INTEGER) is
		do
  	...
  	cc_XSetStipple, x_set_stipple (d, ctxt: POINTER; pid: INTEGER) is
    	external "C use <X11/Xlib.h>"
    	alias "XSetStipple"
    	end

  	cc_XSetTile, x_set_tile (d, ctxt: POINTER; pid: INTEGER) is
    	external "C use <X11/Xlib.h>"
    	alias "XSetTile"
    	end

  	cc_XSetTSOrigin, x_set_ts_origin (d, ctxt: POINTER; x, y: INTEGER) is
    	external "C use <X11/Xlib.h>"
    	alias "XSetTSOrigin"
    	end

  	cc_XSetClipMask, x_set_clip_mask (d, ctxt: POINTER; pid: INTEGER) is
    	external "C use <X11/Xlib.h>"
    	alias "XSetClipMask"
    	end

  	cc_XSetClipOrigin, x_set_clip_origin (d, ctxt: POINTER; x, y: INTEGER) is
    	external "C use <X11/Xlib.h>"
  	...
  	cc_XSetSubwindowMode, x_set_subwindow_mode (d, ctxt: POINTER; mode: INTEGER) is
    	external "C use <X11/Xlib.h>"
    	alias "XSetSubwindowMode"
    	end

  	cc_XSetGraphicsExposures, x_set_graphics_exposures (d, ctxt: POINTER; b: BOOLEAN) is
    	external "C use <X11/Xlib.h>"
    	alias "XSetGraphicsExposures"
    	end

  	cc_XSetRegion, x_set_region (d, ctxt, r: POINTER) is
    	external "C use <X11/Xlib.h>"
    	alias "XSetRegion"
    	end
...
feature -- Drawable routines
	...
	cc_XDrawPoint, x_draw_point (d: POINTER; rid: INTEGER; xgc: POINTER; x, y: INTEGER) is
    	external "C use <X11/Xlib.h>"
    	alias "XDrawPoint"
    	end
