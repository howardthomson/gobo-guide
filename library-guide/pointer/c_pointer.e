...
feature -- get
	...
	get_quad  : INTEGER_64	is require valid_access (8, 0) do Result := c_get_quad	(item, 0) end
	get_float : REAL_32		is require valid_access (4, 0) do Result := c_get_float	(item, 0) end
	get_double: REAL_64		is require valid_access (8, 0) do Result := c_get_double(item, 0) end
	...
	get_quad_at		(offs: INTEGER): INTEGER_64	is require valid_access(8, offs) do Result := c_get_quad	(item, offs) end
	get_float_at	(offs: INTEGER): REAL_32	is require valid_access(4, offs) do Result := c_get_float	(item, offs) end
	get_double_at	(offs: INTEGER): REAL_64	is require valid_access(8, offs) do Result := c_get_double  (item, offs) end

	get_ubyte_at(offs: INTEGER): INTEGER is
...
feature -- put
	...
	put_quad  (v: INTEGER_64)	is require valid_access (8, 0) do c_put_quad	(item, 0, v) end
	put_float (v: REAL_32)		is require valid_access (4, 0) do c_put_float	(item, 0, v) end
	put_double(v: REAL_64)		is require valid_access (8, 0) do c_put_double	(item, 0, v) end
	...
	put_quad_at		(v: INTEGER_64;	offs: INTEGER)	is require valid_access(8, offs) do c_put_quad	(item, offs, v) end
	put_float_at	(v: REAL_32;	offs: INTEGER)	is require valid_access(4, offs) do c_put_float	(item, offs, v) end
	put_double_at	(v: REAL_64;	offs: INTEGER)	is require valid_access(8, offs) do c_put_double(item, offs, v) end

feature -- validation

	valid_if_no_limits: BOOLEAN is True
